...
#include <BipedalLocomotion/Simulator/Simulator.h>
#include <iDynTree/Core/EigenHelpers.h>

using namespace BipedalLocomotion::Simulator;
...
bool Simulator::initialize(std::weak_ptr<ParametersHandler::IParametersHandler> handlerWeak)
{
    ...
    double dT;
    if (!handler->getParameter("sampling_time", dT))
    {
    ...
    m_integrator = std::make_unique<System::ForwardEuler<System::FloatingBaseDynamicalSystem>>(dT);

    // instantiate the dynamical system
    m_dynamicalSystem = std::make_shared<System::FloatingBaseDynamicalSystem>();
                      ...
                      const iDynTree::Transform& rightFootTransform)
{
    ...
    iDynTree::VectorDynSize jointVelocity(m_numberOfDoF);
    jointVelocity.zero();
    ...
    m_gravity(1) = 0;
    m_gravity(2) = -9.81;
    double totalMass = m_kinDyn->model().getTotalMass();

    iDynTree::Position leftFootPosition = leftFootTransform.getPosition();
    leftFootPosition(2) = -totalMass * (-m_gravity(2)) / (2 * m_length * m_width * m_springCoeff);
    ...
    std::cerr << "left " << m_kinDyn->getWorldTransform(m_leftContact.indexInTheModel).toString() << std::endl;
    // set the Dynamical system
    m_dynamicalSystem->setKinDyn(m_kinDyn);
    m_dynamicalSystem->setInitialState({baseTwist.asVector(), jointVelocity,
                basePosition, baseRotation, initialJointValues});
...
bool Simulator::advance(const double& time)
{
    ...
    std::vector<System::ContactWrench> contactWrenches;
    if(m_leftContact.isInContact)
        contactWrenches.push_back(System::ContactWrench(m_leftContact.indexInTheModel,
                                                        m_leftContact.model));
    if(m_rightContact.isInContact)
        contactWrenches.push_back(System::ContactWrench(m_rightContact.indexInTheModel,
                                                        m_rightContact.model));

    m_dynamicalSystem->setControlInput({m_jointTorques, contactWrenches});
    ...
    iDynTree::toEigen(twist.getLinearVec3()) = iDynTree::toEigen(baseTwist).head<3>();
    iDynTree::toEigen(twist.getAngularVec3()) = iDynTree::toEigen(baseTwist).tail<3>();
...
{
    return std::get<4>(m_integrator->getSolution());
}

const iDynTree::VectorDynSize& Simulator::jointVelocities() const
{
    return std::get<1>(m_integrator->getSolution());
}

iDynTree::Transform Simulator::baseTransform() const
{
    const iDynTree::Rotation& baseRotation = std::get<3>(m_integrator->getSolution());
    const iDynTree::Position& basePosition = std::get<2>(m_integrator->getSolution());
    return iDynTree::Transform(baseRotation, basePosition);
}

iDynTree::Twist Simulator::baseVelocity() const
{
    const iDynTree::Vector6& baseTwist = std::get<0>(m_integrator->getSolution());
    iDynTree::Twist twist;
    iDynTree::toEigen(twist.getLinearVec3()) = iDynTree::toEigen(baseTwist).head<3>();
    iDynTree::toEigen(twist.getAngularVec3()) = iDynTree::toEigen(baseTwist).tail<3>();
    return twist;
