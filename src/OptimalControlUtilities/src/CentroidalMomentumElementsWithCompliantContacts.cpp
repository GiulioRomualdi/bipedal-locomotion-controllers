...
CentroidalLinearMomentumElementWithCompliantContact::
    CentroidalLinearMomentumElementWithCompliantContact(
        ...
        const std::vector<std::pair<FrameNames, std::shared_ptr<ContactModels::ContactModel>>>&
            framesInContact)
        : ControlTask(kinDyn)
{
    ...
    {
        FrameInCompliantContact frameInContact;
        const auto& frameName = frame.first;
        const auto& contactModel = frame.second;

        if (m_framesInContact.find(frameName.label()) != m_framesInContact.end())
        {
        ...
        frameInContact.indexRangeInElement = handler.getVariable(frameName.label());
        frameInContact.indexInModel = m_kinDynPtr->model().getFrameIndex(frameName.nameInModel());

        if (!frameInContact.indexRangeInElement.isValid())
            throw std::runtime_error("[CentroidalLinearMomentumElementWithCompliantContact::"
                                     "CentroidalLinearMomentumElementWithCompliantContact] "
                                     "Undefined frame named "
                                     + frameName.label() + " in the variableHandler");

        if (frameInContact.indexInModel == iDynTree::FRAME_INVALID_INDEX)
            throw std::runtime_error("[CentroidalLinearMomentumElementWithCompliantContact::"
                                     "CentroidalLinearMomentumElementWithCompliantContact] "
                                     "Undefined frame named "
                                     + frameName.nameInModel() + " in the model");
        ...
        frameInContact.contactModel = contactModel;

        m_framesInContact.insert({frameName.label(), frameInContact});
    }
}
...
const iDynTree::MatrixDynSize& CentroidalLinearMomentumElementWithCompliantContact::getA()
{
    ...
    for (const auto & frame: m_framesInContact)
    {
        ...
        m_kinDynPtr->getFrameFreeFloatingJacobian(frame.second.indexInModel, m_jacobianMatrix);
        ...
        if (frame.second.isInContact)
            b -= iDynTree::toEigen(frame.second.contactModel->getAutonomousDynamics()).head<3>()
                 + iDynTree::toEigen(frame.second.contactModel->getControlMatrix()).topRows<3>()
                       * iDynTree::toEigen(m_kinDynPtr->getFrameBiasAcc(frame.second.indexInModel));
    }
    return m_b;
}
    ...
    const std::string& name, bool isInContact, const iDynTree::Transform& nullForceTransform)
{
    ...
    if (frame == m_framesInContact.end())
        throw std::runtime_error("[CentroidalLinearMomentumElementWithCompliantContact::"
                                 "setContactState] The frame does not exist.");

    frame->second.isInContact = isInContact;
    if (frame->second.isInContact)
    {
        const auto& indexInModel = frame->second.indexInModel;
